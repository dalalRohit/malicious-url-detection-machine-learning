...
def train(db,test_db):
	...
	df = pandas.read_csv(db) #df
	cols_to_keep,train_cols=return_nonstring_col(df.columns)
	print('\n Training columns: ',train_cols)
	# print('\n\n Train_csv',df)
	# train=df[cols_to_keep]

	# svm_classifier(train_csv,query_csv,train_cols)

	forest_classifier(df,query_csv,train_cols)
